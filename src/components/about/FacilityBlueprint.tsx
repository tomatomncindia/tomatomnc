...
export function FacilityBlueprint() {
  ...
  return (
    <div className="grid items-start gap-8 lg:grid-cols-12 lg:gap-10">
      {/* Blueprint */}
      <div className="border-ink/15 bg-paper-warm relative overflow-hidden border lg:col-span-8">
        {/* Top engineering meta */}
        <div className="border-ink/10 bg-paper-warm/80 text-ink-muted absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-3 border-b px-4 py-2 font-mono text-[10px] tracking-[0.16em] uppercase backdrop-blur-sm">
          <span>FACILITY PLAN · TOMATO M&amp;C INDIA · PYEONGTAEK</span>
          <span className="hidden tabular-nums sm:inline">SCALE 1:N · NTS</span>
        </div>

        {/* Below sm the plan pans horizontally so room codes stay legible. */}
        <div className="overflow-x-auto">
          <svg
            viewBox="0 0 1000 540"
            className="aspect-[1000/540] w-full min-w-[640px] sm:min-w-0"
            role="img"
        ...
        <div className="border-ink/10 text-ink-muted flex items-center justify-between border-t px-4 py-2 font-mono text-[10px] tracking-[0.16em] uppercase">
      ...
      {/* Side detail panel */}
      <div className="space-y-3 lg:sticky lg:top-24 lg:col-span-4">
        <p className="text-forest font-mono text-[10px] tracking-[0.18em] uppercase">AREA LEGEND</p>
        <p className="text-ink-muted text-[13px]">
          Tap or hover a numbered area on the plan to see its function.
        </p>
          ...
          {AREAS.map((a) => {
            ...
            return (
              <li
                ...
                onMouseLeave={() => setActive(null)}
                onClick={() => setActive(isActive ? null : a.id)}
                className={`group cursor-default border-l-2 py-3 pl-4 transition-[border-color,background-color,padding] duration-200 ${
                  isActive ? "border-forest bg-forest/[0.04] pl-5" : "border-line"
                }`}
              >
                ...
                <p className="font-display text-ink mt-1 text-[16px] leading-tight">{a.name}</p>
                <div
                ...
                >
                  <div className="min-h-0">
                    <p className="text-ink-soft text-[12.5px] leading-relaxed">{a.body}</p>
                    <p className="text-ink-muted mt-1.5 font-mono text-[10px] tracking-[0.14em] uppercase tabular-nums">
                      {a.area}
                    </p>
                  </div>
                </div>
